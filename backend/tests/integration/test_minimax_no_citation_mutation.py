...
def test_minimax_tiebreak_does_not_mutate_source_locators() -> None:
    items = [candidate("a", content="证据 A", rrf=0.5), candidate("b", content="证据 B", rrf=0.49)]
    locators = {item.candidate_id: dict(item.source_locator) for item in items}
    ...
    assert all(item.source_locator == locators[item.candidate_id] for item in result.candidates)
