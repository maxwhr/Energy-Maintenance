...
def main() -> int:
    ...
    payload = {
        "generated_at": now_iso(), "probe": "visual", "explicit_cli_gate": args.allow_real_api,
        ...
        "provider": "configured multimodal provider (redacted)", "input_hash": None, "output_hash": None, "latency_ms": None,
    }
    if allowed:
            ...
            if media is None:
                ...
                if user is None or user.role not in {"admin", "expert"}:
                    ...
                    result = MultimodalEvidenceService(db).create_processing_job(media.id, MediaProcessingJobCreate(
                        job_type="multimodal_analysis", provider_code="mimo_2_5", capability="fault_scene_analysis",
                        real_run=True, dry_run=False, mock_run=False, input_summary={"task": "task25c_visual_probe"},
                    ), user)
    ...
    write_json("visual_probe.json", payload)
