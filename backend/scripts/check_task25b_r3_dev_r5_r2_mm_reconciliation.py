...
OUTPUT = ROOT / ".runtime" / "task25b_r3_dev_r5_r2_mm" / "vector_reconciliation.json"
...
def main() -> int:
    ...
    before = json.loads(
        (ROOT / ".runtime" / "task25b_r3_dev_r5_r2_mm" / "r5_r1_snapshot.json").read_text(encoding="utf-8")
    )
    ...
    checks = {
        ...
        "counts_match_frozen_snapshot": counts == (before.get("partition_counts") or {}),
        "full_reindex_disabled": not settings.TASK25B_ALLOW_FULL_REINDEX,
        "default_partition_unchanged": before.get("default_partition_affected") is False,
    }
    passed = all(checks.values())
    payload = {
        "task": "Task 25B-R3-DEV-R5-R2-MM read-only vector reconciliation",
    ...
    print(json.dumps({
        ...
        "partition_counts": counts, "read_only": True,
    }, ensure_ascii=False))
