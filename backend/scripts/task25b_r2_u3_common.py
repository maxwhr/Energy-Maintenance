...
ROOT = BACKEND.parent
RUNTIME = ROOT / ".runtime" / "task25b_r2_u3"
INBOX = BACKEND / "storage" / "formal_corpus_inbox" / "huawei_official"
MANUAL_DOWNLOAD = INBOX / "manual_download"
RUNTIME.mkdir(parents=True, exist_ok=True)
MANUAL_DOWNLOAD.mkdir(parents=True, exist_ok=True)
if str(BACKEND) not in sys.path:
    sys.path.insert(0, str(BACKEND))

OFFICIAL_DOMAINS = {
    "solar.huawei.com", "info.support.huawei.com", "support.huawei.com",
    "download.huawei.com", "support-download.huawei.com", "digitalpower.huawei.com",
    "fusionsolar.huawei.com", "intl.fusionsolar.huawei.com", "huawei.com", "www.huawei.com",
}
...
def sanitize(value: Any, key: str = "") -> Any:
    ...
    if any(term in lowered for term in ("api_key", "authorization", "cookie", "token", "password", "secret")):
        return "[REDACTED]"
    if lowered in {"content", "document_text", "vector", "vectors", "raw_html", "password_hash"}:
        return "[REDACTED]"
