from __future__ import annotations

import argparse
from concurrent.futures import ThreadPoolExecutor

from sqlalchemy import select

from app.core.config import get_settings
from app.core.database import SessionLocal
from app.models import MaintenanceSemanticAnchor
from app.services.vector_index_service import VectorIndexService
from task25b_r3_dev_r4_common import COLLECTION, R4_PARTITION, now_iso, write_json


def partition_count(value: object, partition: str) -> int | None:
    if isinstance(value, dict):
        direct = value.get(partition)
        if isinstance(direct, (int, float, str)):
            try: return int(direct)
            except ValueError: pass
        if isinstance(direct, dict):
            for key in ("count", "doc_count", "total_doc_count", "total"):
                if direct.get(key) is not None: return int(direct[key])
        for nested in value.values():
            found = partition_count(nested, partition)
            if found is not None: return found
    if isinstance(value, list):
        for nested in value:
            found = partition_count(nested, partition)
            if found is not None: return found
    return None


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--allow-real-api", action="store_true")
    parser.add_argument("--partition", required=True)
    args = parser.parse_args()
    if not args.allow_real_api or args.partition != R4_PARTITION:
        raise SystemExit("explicit R4 A/B reconciliation required")
    settings = get_settings()
    with SessionLocal() as db:
        anchors = list(db.scalars(select(MaintenanceSemanticAnchor).where(
            MaintenanceSemanticAnchor.namespace == R4_PARTITION,
            MaintenanceSemanticAnchor.index_status == "active",
        )))
        service = VectorIndexService(db, allow_real_api=True, collection_name=COLLECTION, namespace=R4_PARTITION)
        config = service._runtime_config(provider=settings.EMBEDDING_PROVIDER, vector_backend="dashvector")
        adapter = service._adapter(config)
        batches = [[anchor.vector_id for anchor in anchors[offset:offset + 25]] for offset in range(0, len(anchors), 25)]
        remote = {}
        with ThreadPoolExecutor(max_workers=3) as pool:
            for result in pool.map(adapter.fetch_documents, batches):
                remote.update(result)
        remote_count = partition_count(adapter.collection_stats(), R4_PARTITION)
        missing = sum(anchor.vector_id not in remote for anchor in anchors)
        stale = sum((remote.get(anchor.vector_id, {}).get("fields") or {}).get("content_hash") != anchor.anchor_text_hash for anchor in anchors if anchor.vector_id in remote)
        duplicate = len(anchors) - len({anchor.vector_id for anchor in anchors})
        language = sum(anchor.language != "zh-CN" for anchor in anchors)
        status = sum(((anchor.semantic_fields or {}).get("semantic_unit") or {}).get("quality_status") != "ENGINEERING_VERIFIED_SOURCE_GROUNDED" for anchor in anchors)
        current = sum(not anchor.current_version for anchor in anchors)
        metadata_mapping = sum(
            not ((remote.get(anchor.vector_id, {}).get("fields") or {}).get("semantic_unit_id")) for anchor in anchors if anchor.vector_id in remote
        )
        orphan = max(0, int(remote_count or 0) - len(anchors)) if remote_count is not None else None
        checks = {
            "missing_zero": missing == 0, "orphan_zero": orphan == 0, "duplicate_zero": duplicate == 0,
            "hash_mismatch_zero": stale == 0, "language_zero": language == 0, "quality_zero": status == 0,
            "current_zero": current == 0, "metadata_mapping_zero": metadata_mapping == 0,
        }
        payload = {
            "generated_at": now_iso(), "collection": config["collection_name"], "partition": R4_PARTITION,
            "semantic_units": len({((anchor.semantic_fields or {}).get("semantic_unit_id")) for anchor in anchors}),
            "anchor_vectors": len(anchors), "remote_partition_count": remote_count,
            "missing": missing, "orphan": orphan, "duplicate": duplicate, "hash_mismatch": stale,
            "language_leakage": language, "quality_leakage": status, "current_leakage": current,
            "metadata_mapping_failure": metadata_mapping, "pilot_r2_changed": False, "pilot_r3_changed": False,
            "default_partition_changed": False, "vectors_exported": False, "checks": checks, "passed": all(checks.values()),
        }
    write_json("semantic_unit_reconciliation.json", payload)
    print({"status": "PASSED" if payload["passed"] else "FAILED", **{key: payload[key] for key in ("anchor_vectors", "remote_partition_count", "missing", "orphan", "duplicate", "hash_mismatch")}})
    if not payload["passed"]:
        raise SystemExit(2)


if __name__ == "__main__":
    main()
