...
from task25g_r1_common import TASK25G_RUNTIME, now_iso, write_json
...
def main() -> int:
    ...
    queries = {
        ...
        "two_hop": (
            "SELECT e2.id FROM kg_edges e1 JOIN kg_edges e2 ON e1.target_node_id=e2.source_node_id "
            "WHERE e1.status='active' AND e2.status='active' LIMIT 200",
            {},
        ),
    }
    ...
    with SessionLocal() as session:
        for _ in range(30):
            for name, (statement, params) in queries.items():
                ...
                except Exception as exc:  # noqa: BLE001 - performance gate records query errors.
        ...
        for _ in range(30):
            ...
            except Exception as exc:  # noqa: BLE001 - performance gate records service errors.
    ...
    failures = [name for name, threshold in thresholds.items() if metrics[name]["p95_ms"] > threshold]
    if max(sql_counts or [0]) > 25:
        failures.append("sql_count")
    if errors:
        failures.append("query_error")
    baseline = json.loads((TASK25G_RUNTIME / "kg_query_performance.json").read_text(encoding="utf-8"))
    payload = {
        "version": "task25g_r1_kg_performance_preservation_v1",
        ...
        "task25g_baseline_metrics": baseline.get("metrics") or {},
        "errors": errors,
        "failures": failures,
    }
    write_json("kg_performance_preservation.json", payload)
    print(
        json.dumps(
            {
                "status": payload["status"],
                "p95_ms": {key: value["p95_ms"] for key, value in metrics.items()},
                "sql_count_max": payload["sql_count_max"],
            },
            ensure_ascii=False,
        )
    )
