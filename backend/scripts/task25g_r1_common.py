...
ROOT = BACKEND.parent
RUNTIME = ROOT / ".runtime" / "task25g_r1"
...
def write_json(path: str | Path, payload: Any, *, overwrite: bool = True) -> Path:
    ...
    if target.exists() and not overwrite:
        raise RuntimeError(f"refusing to overwrite frozen Task 25G-R1 evidence: {target.name}")
...
def table_columns(session: Any, table_name: str) -> set[str]:
    from sqlalchemy import text
...
def scalar_count(session: Any, statement: str, params: dict[str, Any] | None = None) -> int:
    from sqlalchemy import text

    return int(session.scalar(text(statement), params or {}) or 0)
