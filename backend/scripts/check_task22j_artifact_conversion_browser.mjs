...
const BASE_URL = (process.env.TASK22J_BASE_URL || "http://127.0.0.1:8010").replace(/\/$/, "");
const API_BASE_URL = `${BASE_URL}/api`;
const ADMIN_USERNAME = process.env.TASK22J_ADMIN_USERNAME || "admin";
const ADMIN_PASSWORD = process.env.TASK22J_ADMIN_PASSWORD || "admin123456";
const VIEWER_USERNAME = process.env.TASK22J_VIEWER_USERNAME || `Task22J_viewer_${Date.now()}`;
const VIEWER_PASSWORD = process.env.TASK22J_VIEWER_PASSWORD || "Task22J_pass123";
const CDP_PORT = Number(process.env.TASK22J_CDP_PORT || 9231);
const RUN_ID = `Task22J_${new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 14)}`;
const RUNTIME_DIR = path.resolve(process.cwd(), ".runtime", "task22j");
const RESULT_FILE = path.join(RUNTIME_DIR, "artifact_conversion_browser_result.json");
...
const browserCandidates = [
  process.env.TASK22J_BROWSER_PATH,
...
async function ensureViewer() {
  ...
  const payload = { password: VIEWER_PASSWORD, display_name: "Task22J Viewer", role: "viewer", status: "active" };
  if (existing) {
...
async function createDevice() {
  const payload = {
    ...
    location: "Task22J browser bay",
    status: "fault",
    metadata_json: { marker: RUN_ID },
    description: `${RUN_ID} browser artifact conversion device.`,
  };
...
async function launchBrowser() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "em-task22j-profile-"));
...
async function fillWorkbench(agentCode, inputText) {
...
async function createRunViaUi(agentCode, artifactType, approvalType) {
  await fillWorkbench(agentCode, `${RUN_ID} ${agentCode} artifact conversion browser validation.`);
  await cdp.eval(domScript(`
  ...
  await waitFor(async () => {
    ...
    const approvalReady = approvalType
      ? await cdp.eval(`Boolean(document.querySelector(${JSON.stringify(`[data-testid="approve-${approvalType}"]`)}))`)
      : true;
    return text.includes(artifactType) && approvalReady;
  }, 70000);
  record(`browser create ${agentCode}`, "passed", artifactType);
}

async function approveApproval(approvalType) {
  await waitFor(() => cdp.eval(domScript(`
    const button = document.querySelector(${JSON.stringify(`[data-testid="approve-${approvalType}"]`)});
  ...
  await waitFor(async () => {
    const text = await bodyText();
    return text.includes(`${approvalType}`) && text.includes("approved");
  }, 20000);
  record(`browser approve ${approvalType}`, "passed");
}

async function convertVisible(targetType) {
  await waitFor(async () => cdp.eval(`Boolean(document.querySelector('[data-testid="convert-${targetType}"]'))`), 20000);
  await cdp.eval(domScript(`
    for (const checkbox of Array.from(document.querySelectorAll("input[type='checkbox']")).filter(visible)) {
      if (!checkbox.checked) checkbox.click();
    }
    const button = document.querySelector("[data-testid='convert-${targetType}']");
    if (!button || button.disabled) return false;
    button.click();
    return true;
  `));
  await waitFor(async () => {
    const text = await bodyText();
    return text.includes("conv-") && text.includes(targetType.replace("_", " ").split(" ")[0]);
  }, 30000);
  record(`browser convert ${targetType}`, "passed");
}
...
async function verifyAdminFlow() {
  ...
  await createRunViaUi("sop_planner_agent", "sop_draft", "sop_draft_review");
  await approveApproval("sop_draft_review");
  await convertVisible("sop_template");

  await createRunViaUi("task_orchestration_agent", "task_draft", "task_draft_review");
  await approveApproval("task_draft_review");
  await convertVisible("maintenance_task");

  await createRunViaUi("knowledge_curator_agent", "knowledge_contribution_draft", "knowledge_contribution_draft_review");
  await approveApproval("knowledge_contribution_draft_review");
  await convertVisible("knowledge_contribution");
  await convertVisible("kg_candidate");
}
...
async function verifyViewerReadonly() {
  ...
  record("viewer readonly conversion UI", "passed", "no conversion buttons and create disabled");
}
...
main()
  .catch((error) => {
    record("Task22J browser acceptance", "failed", error.stack || error.message);
  })
  .finally(async () => {
    await shutdown();
    fs.mkdirSync(RUNTIME_DIR, { recursive: true });
